...
interface ImagePreviewModalProps {
  items: Array<{ src: string; prompt?: string; }>;
  startIndex?: number;
  onClose: () => void;
  isEditable?: boolean;
...
const ImagePreviewModal: React.FC<ImagePreviewModalProps> = ({ items, startIndex = 0, onClose, isEditable = false, onEdit, onDescribe, remainingDescribes }) => {
  ...
  const goToNext = useCallback(() => {
    if (isEditing || isDescribing) return;
    setCurrentIndex((prevIndex) => (prevIndex + 1) % items.length);
  }, [items.length, isEditing, isDescribing]);
  ...
  }, [onClose, goToNext, goToPrev, items.length, isEditing]);
